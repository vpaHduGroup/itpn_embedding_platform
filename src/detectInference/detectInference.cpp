...
DetectInferenceThread::DetectInferenceThread(string modelPath)
    :model_(modelPath), isReleased(false),prevInferenceOutput_()
{
}
...
{
    if (detectDataMsg->usePreviousResult) {  // 使用前一帧的推理结果
        detectDataMsg->inferenceOutput = prevInferenceOutput_;  // 将前一帧的结果赋值给当前帧
        return ACLLITE_OK;
    } else {
    	...
    	model_.DestroyInput();
	prevInferenceOutput_ = detectDataMsg->inferenceOutput;
    }
    return ACLLITE_OK;
}
