...
namespace {
    ...
    const vector <cv::Scalar> kColors {cv::Scalar(50, 205, 50)};
...
AclLiteError DetectPostprocessThread::InferOutputProcess(shared_ptr<DetectDataMsg> detectDataMsg)
{
    ...
    if (detectDataMsg->usePreviousResult) {
        return ACLLITE_OK;
    }
    size_t pos = 0;
    for (int n = 0; n < detectDataMsg->decodedImg.size(); n++) {
        ...
        for (size_t i = 0; i < modelOutputBoxNum; ++i) {
            float maxValue = 0;
            float maxIndex = 0;
            for (size_t j = offset; j < totalNumber; ++j) {
                float value = detectBuff[i * totalNumber + j];
                    if (value > maxValue) {
                    // index of class
                    maxIndex = j - offset;
                    maxValue = value;
        ...
        for (size_t i = 0; i < result.size(); ++i) {
            ...
            std::stringstream ss;
	    ss << std::fixed << std::setprecision(3) << result[i].score;
	    className = label[result[i].classIndex] + ":" + ss.str();
            cv::rectangle(detectDataMsg->frame[n], leftTopPoint,
                rightBottomPoint, kColors[0], kLineSolid);
    ...
    // 计算后处理时间（以毫秒为单位）
    long postproc_duration = (end_time.tv_sec - start_time.tv_sec) * 1000 + (end_time.tv_usec - start_time.tv_usec) / 1000;

    // 输出后处理时间
    std::cout << "Postprocessing time: " << postproc_duration << " ms" << std::endl;
...
{
    if (detectDataMsg->usePreviousResult) {  // 使用前一帧的结果，直接发送
        while (1) {
            AclLiteError ret = SendMessage(
                detectDataMsg->dataOutputThreadId,
                MSG_OUTPUT_FRAME,
                detectDataMsg
            );
            if (ret == ACLLITE_ERROR_ENQUEUE) {
                ...
                ACLLITE_LOG_ERROR("Send message failed, error %d", ret);
                return ret;
            }
        }
    } else {
	if (!sendLastBatch_) {
            // 发送普通帧数据（MSG_OUTPUT_FRAME）
            while (1) {
